...
function UniformModal({ uniform, onClose, onSave, authFetch }) {
  const [form, setForm] = useState(
    uniform || { uniform_type: "", description: "", image_path: "" },
  );
  ...
  const handleSave = async () => {
    if (!form.uniform_type.trim()) {
      setError("Uniform type is required");
      return;
    }
    setLoading(true);
    try {
      const url = uniform
        ? `/api/admin/uniforms/${uniform.uniform_id}`
        : "/api/admin/uniforms";
      ...
      <div className="modal">
        ...
        <div className="field">
          ...
          <input
            ...
            onChange={set("uniform_type")}
            placeholder="e.g. Proper Uniform"
          />
        </div>
        <div className="field">
          ...
          <textarea
            ...
            placeholder="Describe the uniform..."
          />
        </div>
        <div className="field">
          <label>Image</label>
          <input
            ...
            placeholder="/static/images/filename.png"
          />
...
function PriceModal({ price, uniforms, onClose, onSave, authFetch }) {
  const [form, setForm] = useState(
    price || {
      uniform_id: uniforms[0]?.uniform_id || "",
      label: "S",
      amount: "",
    },
  );
  ...
  const handleSave = async () => {
    ...
    try {
      const url = price
        ? `/api/admin/prices/${price.price_id}`
        : "/api/admin/prices";
    ...
    <div className="admin-card">
      <div className="admin-card-price-row">
        <div>
          ...
          <div className="admin-card-size">
            Size: <strong>{p.label}</strong>
          </div>
        </div>
      ...
      <div className="page-content">
        ...
        {tab === "uniforms" && (
          <div className="fade-up">
                    ...
                    <tr key={u.uniform_id}>
                      ...
                      <td>
                        <div className="actions">
                          ...
                          <button
                            className="btn btn-danger btn-sm"
                            onClick={() => deleteUniform(u.uniform_id)}
                          >
                            Delete
                          </button>
                        </div>
            ...
            <div className="admin-cards">
              ...
              {uniforms.length === 0 && (
                <p className="admin-empty">No uniforms yet.</p>
              )}
            </div>
          </div>
        )}
        ...
        {tab === "prices" && (
          <div className="fade-up">
            ...
            <div className="admin-cards">
              ...
              {allPrices.length === 0 && (
                <p className="admin-empty">No prices yet.</p>
              )}
            </div>
          </div>
        )}
        ...
        {tab === "students" && (
          <div className="fade-up">
            ...
            <div className="admin-table-wrap">
              <table className="data-table">
                ...
                <tbody>
                  ...
                  {students.length === 0 &&
                    EMPTY(4, "No students registered yet.")}
                </tbody>
              </table>
            </div>
            ...
            <div className="admin-cards">
              {students.map((s) => (
                <StudentCard key={s.student_id} s={s} />
              ))}
              {students.length === 0 && (
