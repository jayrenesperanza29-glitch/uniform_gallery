...
export default function UniformDetail() {
  ...
  return (
    <div className="app-shell">
      <Topbar />
      <div className="page-content">
        <Link to="/gallery" className="back-link fade-up">← Back to Gallery</Link>

        <div className="detail-grid">
          {/* Image */}
          <div className="fade-up fade-up-1">
            {uniform.image_path
              ? <img src={uniform.image_path} alt={uniform.uniform_type} className="detail-img"
                     onError={e => { e.target.style.display='none'; e.target.nextSibling.style.display='flex' }} />
              : null
            }
            <div className="detail-img-placeholder" style={{ display: uniform.image_path ? 'none' : 'flex' }}>
              {icon}
            </div>
          </div>
          ...
          <div className="detail-info fade-up fade-up-2">
            ...
            {uniform.prices?.length > 0 && (
              <>
                <h3 style={{ fontFamily: 'var(--font-head)', fontSize: '1.15rem', marginBottom: 14 }}>
                  Price List by Size
                </h3>
                  ...
                  <tbody>
                    {uniform.prices.map(p => (
                      <tr key={p.price_id}>
                        ...
                        <td className="amt">₱ {parseFloat(p.amount).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <Link to="/dashboard" className="btn btn-secondary" style={{ display:'inline-flex', textDecoration:'none' }}>
              ← Back to Dashboard
            </Link>
          </div>
        </div>
